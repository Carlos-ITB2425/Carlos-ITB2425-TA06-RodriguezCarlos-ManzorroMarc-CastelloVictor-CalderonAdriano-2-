...
ruta_log = 'Prueba'  # Cambia esto por la ruta deseada
...
with open(archivo_resultados, 'a', encoding='utf-8') as log:
...
# Generar gráfico de las estadísticas anuales
plt.figure(figsize=(15, 10))  # Aumentar el tamaño de la figura
plt.bar(medias_anuales_por_anio.keys(), medias_anuales_por_anio.values())
plt.title('Media Anual de Precipitación por Año')
plt.xlabel('Año')
plt.ylabel('Media Anual de Precipitación (L/m²)')
plt.tight_layout()
plt.savefig('media_anual_precipitacion.png')
plt.close()

print(f"Resultados globales guardados en: {archivo_resultados}")
print("Gráfico de estadísticas anuales guardado en: media_anual_precipitacion.png")